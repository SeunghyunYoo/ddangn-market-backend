...
public class UploadFileController {
    ...
    @PostMapping
    public ResponseEntity<ResponseOKDto<UploadFileResponseDto>> uploadImage(@RequestParam List<MultipartFile> files, @ApiIgnore HttpSession session) throws IOException {
        accountService.checkSessionAndFindAccount(session);

        List<Long> fileIds = uploadFileService.uploadFile(files);
